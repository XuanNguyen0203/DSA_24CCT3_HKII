...
void removeTail(List*& L)
{
    ...
    NODE* curNode = L->p_head;
    while (curNode->p_next->p_next != nullptr) {
        curNode = curNode->p_next;
    }
    L->p_tail = curNode;
    delete curNode->p_next;
    curNode->p_next = nullptr;
    return;
}
...
void removeBefore(List*& L, int val)
{
    ...
    if (L->p_head->p_next != nullptr && L->p_head->p_next->p_next == nullptr && L->p_head->p_next->key == val) {
        removeHead(L);
        return;
    }
    NODE* preNode = L->p_head;
    while (preNode->p_next != nullptr && preNode->p_next->p_next != nullptr && preNode->p_next->p_next->key != val) {
        preNode = preNode->p_next;
    }
    if (preNode->p_next == nullptr || preNode->p_next->p_next == nullptr) {
        return;
    }
    NODE* temp = preNode->p_next->p_next;
    delete preNode->p_next;
    preNode->p_next = temp;
    return;
}
...
void removeAfter(List*& L, int val)
{
    ...
    if (L->p_head->p_next != nullptr && L->p_head->p_next->p_next == nullptr && L->p_head->key == val) {
        removeTail(L);
        return;
    }
    NODE* curNode = L->p_head;
    while (curNode != nullptr && curNode->key != val) {
        curNode = curNode->p_next;
    }
    if (curNode == nullptr || curNode->p_next == nullptr) {
        return;
    }
    if (curNode->p_next == L->p_tail) {
        removeTail(L);
        return;
    }
    NODE* temp = curNode->p_next->p_next;
    delete curNode->p_next;
    curNode->p_next = temp;
    return;
}
...
bool addPos(List*& L, int data, int pos)
{
    ...
    NODE* preNode = L->p_head;
    int Pos = 0;
    while (preNode != nullptr && Pos < pos - 1) {
        preNode = preNode->p_next;
        Pos++;
    }
    if (preNode == nullptr) {
...
void removePos(List*& L, int data, int pos)
{
    ...
    int Pos = 0;
    NODE* curNode = L->p_head;
    while (curNode != nullptr && Pos < pos - 1) {
        Pos++;
        curNode = curNode->p_next;
    }
    if (curNode == nullptr || curNode->p_next == nullptr) {
    ...
    if (curNode->p_next->p_next == nullptr) {
        removeTail(L);
        return;
    }
    NODE* temp = curNode->p_next->p_next;
...
{
    NODE* p = createNode(data);
    if (L == nullptr || L->p_head == nullptr || p == nullptr) {
        return false;
    }
    if (L->p_head->key == val) {
        addHead(L, data);
    ...
    NODE* preNode = L->p_head;
    while (preNode->p_next != nullptr && preNode->p_next->key != val) {
        preNode = preNode->p_next;
    }
    if (preNode->p_next == nullptr) {
        return false;
    }
    NODE* temp = preNode->p_next;
    preNode->p_next = p;
    p->p_next = temp;
    return true;
}
...
bool addAfter(List*& L, int data, int val)
{
    ...
    if (curNode->p_next == nullptr && curNode->key == val) {
        addTail(L, data);
        return true;
    }
    NODE* temp = curNode->p_next;
    curNode->p_next = p;
    p->p_next = temp;
    return true;
}
...
void removeDuplicate(List*& L)
{
    ...
    for (NODE* p = L->p_head; p != nullptr; p = p->p_next) {
        NODE* curNode = p;
        NODE* runNode = curNode->p_next;
        while (runNode != nullptr) {
            if (p->key == runNode->key) {
                curNode->p_next = runNode->p_next;
                delete runNode;
                runNode = curNode->p_next;
            }
            else {
                curNode = runNode;
...
bool removeElement(List*& L, int key)
{
    ...
    if (L->p_head->p_next == nullptr && L->p_head->key != key) {
        return false;
    }
    if (L->p_head->key == key) {
        removeHead(L);
    ...
    NODE* preNode = L->p_head;
    while (preNode != nullptr && preNode->p_next != nullptr && preNode->p_next->key != key) {
        preNode = preNode->p_next;
    }
    if (preNode == nullptr || preNode->p_next == nullptr) {
        return false;
    }
    NODE* temp = preNode->p_next;
    preNode->p_next = temp->p_next;
    if (temp == L->p_tail) {
        L->p_tail = preNode;
    }
    delete temp;
