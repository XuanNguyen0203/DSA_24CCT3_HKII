...
List* createList(NODE* p_node)
{
    ...
    if (l != nullptr) {
        ...
        p_node->p_next = p_node->p_pre = nullptr;
    }
    return l;
}
...
void removeBefore(List*& L, int val)
{
    if (L == nullptr || L->p_head == nullptr || L->p_head == L->p_tail) return;
    ...
    if (curNode == nullptr || curNode->p_pre == nullptr || curNode->p_pre->p_pre == nullptr) return;

    NODE* temp = curNode->p_pre->p_pre;
    delete curNode->p_pre;
    curNode->p_pre = temp;
    temp->p_next = curNode;
}
void removeAfter(List*& L, int val)
{
    ...
    if (curNode == nullptr || curNode->p_next == nullptr) return;
...
void removePos(List*& L, int data, int pos)
{
    ...
    NODE* tempNext = curNode->p_next;
    NODE* tempPre = curNode->p_pre;
    delete curNode;
    tempPre->p_next = tempNext;
    tempNext->p_pre = tempPre;
    return;
}
...
bool addAfter(List*& L, int data, int val)
{
    ...
    if (curNode == L->p_tail && curNode->key == val) {
        addTail(L, data);
        return true;
    }
...
List* reverseList(List* L)
{
    ...
    while (curNode != nullptr) {
        temp = curNode->p_next;
        curNode->p_next = curNode->p_pre;
        curNode->p_pre = temp;
        curNode = curNode->p_pre;
    }
    ...
    for (NODE* p = L->p_head; p != nullptr; p = p->p_next) {
        ...
        while (runNode != nullptr) {
            if (p->key == runNode->key) {
                NODE* temp = runNode->p_next;
                curNode->p_next = temp;
                delete runNode;
                runNode = temp;
                if (runNode != nullptr) {
                    runNode->p_pre = curNode;
                }
                else {
                    L->p_tail = curNode;
                }
